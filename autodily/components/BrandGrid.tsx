...
const BRANDS: Array<{ name: string; displayName?: string }> = [
  { name: "AUDI" }, { name: "BMW" }, { name: "CITROEN", displayName: "Citroën" },
  { name: "DACIA" }, { name: "FIAT" }, { name: "FORD" }, { name: "HONDA" },
  { name: "HYUNDAI" }, { name: "KIA" }, { name: "MAZDA" },
  { name: "MERCEDES-BENZ", displayName: "Mercedes" }, { name: "NISSAN" },
  { name: "OPEL" }, { name: "PEUGEOT" }, { name: "RENAULT" }, { name: "SEAT" },
  { name: "SKODA", displayName: "Škoda" }, { name: "TOYOTA" },
  { name: "VOLVO" }, { name: "VW", displayName: "Volkswagen" },
];

export default function BrandGrid() {
  const [, setBrands] = useState<BrandItem[]>([]);
  useEffect(() => { fetch("/api/vehicles?action=brands").then((r) => r.json()).then(setBrands).catch(() => {}); }, []);
      ...
      {BRANDS.map((b) => (
        ...
        >
          <img
            ...
            loading="lazy"
          />
          <span className="text-mltext-light text-[9px] font-bold uppercase tracking-wider text-center leading-tight group-hover:text-mltext-dark transition-colors">
